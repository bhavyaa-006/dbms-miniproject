  ...
  return (
    <div className={compact ? 'rounded-xl border border-white/5 bg-surface-2/80 p-4' : 'card text-center py-12'}>
      <div className={compact ? 'flex items-start gap-3' : 'flex flex-col items-center gap-3'}>
        {Icon && (
          <div className={compact ? 'mt-0.5 text-zinc-400' : `p-3 rounded-2xl ${isError ? 'bg-red-500/10 text-red-400' : 'bg-white/5 text-zinc-400'}`}>
            <Icon size={compact ? 18 : 28} />
          </div>
        )}
        <div className={compact ? 'flex-1' : 'max-w-sm'}>
